...
# Garantir que o projeto raiz esteja no path para importar `config.settings`.
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'config.settings')
django.setup()

# Permitir host de teste usado pelo Django test client
try:
...
# Testar criação via API (permite AllowAny em create)
payload = json.dumps({
	'username': 'apiuser',
